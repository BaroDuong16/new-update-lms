...
#     current_user = request.user  # Current logged-in user
#     sender_name = request.GET.get('sender', None)
...
#     if request.method == "POST":
#         print(request.POST)  # Debugging to see what data is sent
#         message_text = request.POST.get('message', '')
#         selected_sender = request.POST.get('sender', current_user.username)  # Use current user as sender
...
#         'sender': sender,
#         'users': users,
#         'user': request.user  # Pass the current logged-in user
#     }
#     return render(request, 'chat/chat_view.html', context)
